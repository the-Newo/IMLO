...
train_transform = transforms.Compose([
    ...
    transforms.RandomResizedCrop(64, scale=(0.8, 1.0), ratio=(0.75, 1.333)),
    transforms.GaussianBlur(3, sigma=(0.1, 2.0)),  # Add GaussianBlur with random kernel size and sigma
    transforms.RandomPerspective(distortion_scale=0.2, p=0.5),  # Add RandomPerspective
    transforms.RandomGrayscale(p=0.2),  # Add RandomGrayscale
...
class ImprovedNeuralNetwork(nn.Module):
    def __init__(self):
        ...
        self.fc_stack = nn.Sequential(
            nn.Linear(2048 * 4 * 4, 1024),  # Adjusted input size
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(1024, 512),
            nn.ReLU(),
